function searchGoogleBooks() {
    ...
    fetch('https://www.googleapis.com/books/v1/volumes?q=' + txtSearch.value + "&startIndex=0&maxResults=20")
        ...
        .then(json => {
            var list = `<div class="row col-8 center">found ${json.totalItems} books</div> <div class="row">`
            for (let i = 0; i < json.items.length; i++) {
                console.log(json.items[i].volumeInfo.title)
                var bookInfo = json.items[i].volumeInfo;
                var template = `<ul class="col-sm-12 col-md-4 col-xl-3>`;
                template += `<a class="list-group-item list-group-item-action active pretty-item" href="${json.items[i].selfLink}">${bookInfo.title}</a>`
